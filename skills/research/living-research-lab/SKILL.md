...
research_db(
  ...
  data={
    ...
    "tags": ["<relevant-tag>", ...],
    "engagement": <stars_or_upvotes_integer>
  }
)
```

Save ALL findings even if similar ones were found before. Trust scores are auto-computed from the source URL domain.

---

### Step 4.5 — Score & Rank Findings (Signal vs Noise)

```
research_db(action="score_findings", topic="<TOPIC>", days=7)
```

This returns findings ranked by a composite score (trust × recency × engagement). Use the **top-ranked findings** for the Executive Summary — they represent the highest-signal items.

---

### Step 4.7 — Detect Anomalies

```
research_db(action="detect_anomalies", topic="<TOPIC>", days=7)
```
...
### Step 7.5 — Auto-Topic Evolution

```
research_db(action="suggest_subtopics", topic="<TOPIC>", days=14)
```
...
```
research_db(action="add_topic", topic="<suggested subtopic>", data={"description": "Auto-evolved from '<TOPIC>' — [tag] appeared N times"})
```

3. Tell the user: "I found a recurring pattern and started tracking '<subtopic>' as a separate research thread."
